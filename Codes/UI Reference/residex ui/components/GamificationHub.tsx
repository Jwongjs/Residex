  ...
  const agents: AgentSlide[] = [
    {
      ...
      mainAsset: (
        <div className="relative w-64 h-64">
           ...
           <div className="relative w-full h-full border-4 border-indigo-400 p-2 rotate-3 animate-float bg-slate-900 overflow-hidden shadow-[0_0_50px_rgba(99,102,241,0.4)]">
              {user.profileImage ? (
                  <img src={user.profileImage} alt={user.name} className="w-full h-full object-cover grayscale contrast-125" />
              ) : (
                  <div className="w-full h-full flex items-center justify-center bg-slate-800 text-6xl font-black text-indigo-300">
                    {user.avatarInitials}
                  </div>
              )}
              <div className="absolute top-0 right-0 bg-indigo-500 text-white px-2 py-1 font-black text-xs">RANK #{user.stats?.ranking || 47}</div>
           </div>
        </div>
      ),
      stats: [
    ...
    },
    {
      ...
      mainAsset: (
        <div className="scale-[2.5] animate-float drop-shadow-[0_0_30px_rgba(251,191,36,0.4)]">
    ...
    },
    {
      ...
      mainAsset: (
        <div className="scale-[2.5] animate-float drop-shadow-[0_0_30px_rgba(203,213,225,0.4)]">
  ...
  return (
    <div className={`flex flex-col h-full bg-black font-sans relative overflow-hidden select-none`}>
      ...
      <div className="absolute bottom-1 left-1/2 -translate-x-1/2 text-[8px] font-black opacity-30 uppercase tracking-[0.3em] italic pointer-events-none">
          RESIDEX PROTOCOL // SYSTEM_ONLINE
      </div>
    </div>
  );
};
