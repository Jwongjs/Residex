...
export const LiquidityOverview: React.FC<LiquidityOverviewProps> = ({ groups, onBack, onViewDetails }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#000402] animate-in slide-in-from-bottom-6 duration-500 relative overflow-hidden">
      {/* Ambient Glow */}
      <div className="absolute top-0 left-0 right-0 h-[500px] bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-emerald-900/60 via-black to-black pointer-events-none"></div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar pb-32 relative z-10">
        {mockPools.map((pool, index) => {
          ...
          return (
            <div key={pool.id} className="relative group">
                <div className="absolute -inset-0.5 bg-gradient-to-r from-emerald-600 to-teal-600 rounded-[2rem] opacity-20 group-hover:opacity-40 blur transition duration-500"></div>
                <button 
                onClick={() => onViewDetails(pool.groupId)}
                className="w-full text-left bg-slate-900/60 border border-white/5 rounded-[2rem] p-5 hover:border-emerald-500/30 transition-all group overflow-hidden relative animate-in slide-in-from-bottom-4 backdrop-blur-md"
                style={{ animationDelay: `${index * 100}ms` }}
                >
                {/* Background Progress Glow */}
                <div 
                    className="absolute left-0 bottom-0 h-1 bg-emerald-500/30 transition-all duration-1000" 
                    style={{ width: `${percent}%` }}
                ></div>
                ...
                <div className="flex items-center justify-between gap-4 relative z-10">
                    ...
                    <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                        <div 
                            className="h-full bg-emerald-500 shadow-[0_0_10px_rgba(52,211,153,0.6)] transition-all duration-1000" 
                            style={{ width: `${percent}%` }}
                        ></div>
                    </div>
                    </div>
