...
export const PaymentsDueDetail: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  return (
    <div className="flex flex-col h-full bg-[#050001] animate-in slide-in-from-bottom-6 duration-500 relative overflow-hidden">
      {/* Ambient Rose Glow */}
      <div className="absolute top-0 left-0 right-0 h-[500px] bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-rose-900/60 via-black to-black pointer-events-none"></div>

      <div className="p-6 flex items-center gap-4 border-b border-rose-500/10 sticky top-0 bg-[#050001]/95 backdrop-blur-md z-10">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar pb-24 relative z-10">
        <div className="relative group">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-rose-600 to-red-600 rounded-[2.5rem] opacity-20 group-hover:opacity-40 blur transition duration-500"></div>
            <div className="relative bg-slate-900/60 border border-rose-500/20 rounded-[2.5rem] p-8 text-center overflow-hidden backdrop-blur-md">
            <div className="absolute top-0 right-0 p-4 opacity-10"><CreditCard size={120} className="text-rose-500" /></div>
            <div className="relative z-10">
                <div className="h-12 w-12 rounded-full bg-rose-500/20 flex items-center justify-center text-rose-500 mx-auto mb-4 animate-pulse border border-rose-500/30">
                    <AlertCircle size={28} />
                </div>
          ...
          <div className="space-y-4">
             {[
               ...
               <div key={i} className="relative group">
                   <div className="absolute -inset-0.5 bg-gradient-to-r from-rose-600 to-orange-600 rounded-[2rem] opacity-15 group-hover:opacity-30 blur transition duration-500"></div>
                   <div className="relative bg-slate-900/60 border border-rose-500/20 p-5 rounded-[2rem] hover:bg-slate-900/80 transition-all backdrop-blur-md">
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <h4 className="text-white font-bold text-sm mb-1 group-hover:text-rose-200 transition-colors">{payment.title}</h4>
                    ...
                    <div className="flex gap-2">
                        <button className="flex-1 py-3 bg-rose-600 text-white font-black text-[10px] uppercase rounded-xl shadow-lg shadow-rose-900/30 active:scale-95 transition-all flex items-center justify-center gap-2 hover:bg-rose-500">
        ...
        <section>
          ...
          <div className="space-y-3 opacity-60">
             {[
               ...
               <div key={i} className="flex justify-between items-center p-4 bg-slate-900/30 rounded-2xl border border-white/5 hover:bg-slate-800/50 transition-colors">
      ...
      <div className="absolute bottom-6 left-6 right-6 z-20">
         <button className="w-full bg-white/5 backdrop-blur-xl border border-white/10 p-4 rounded-2xl flex items-center justify-center gap-3 text-slate-400 hover:text-white transition-all active:scale-[0.98] group">
            <div className="group-hover:text-rose-400 transition-colors"><Calendar size={18} /></div>
