...
export const GroupLiquidityDetail: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  return (
    <div className="flex flex-col h-full bg-[#000402] animate-in slide-in-from-bottom-6 duration-500 relative overflow-hidden">
      {/* Ambient Glow */}
      <div className="absolute top-0 left-0 right-0 h-[500px] bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-emerald-800/50 via-black to-black pointer-events-none"></div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar pb-20 relative z-10">
        <div className="relative group">
            <div className="absolute -inset-0.5 bg-gradient-to-br from-emerald-600 to-teal-600 rounded-[2.5rem] opacity-20 group-hover:opacity-30 blur transition duration-500"></div>
            <div className="relative bg-emerald-900/10 border border-emerald-500/20 rounded-[2.5rem] p-8 text-center backdrop-blur-md">
            ...
            <div className="w-full h-2 bg-slate-900 rounded-full mt-6 overflow-hidden">
                <div className="h-full bg-emerald-500 w-[66.7%] shadow-[0_0_20px_rgba(16,185,129,0.6)]"></div>
            </div>
            </div>
        </div>
          ...
          <div className="space-y-4">
             {[
               ...
               <div key={i} className="relative group">
                   <div className="absolute -inset-0.5 bg-gradient-to-r from-emerald-500 to-green-500 rounded-2xl opacity-10 group-hover:opacity-30 blur transition duration-500"></div>
                   <div className="relative bg-slate-900/60 border border-white/5 p-5 rounded-2xl overflow-hidden backdrop-blur-sm">
                        <div className="flex justify-between items-start mb-4 relative z-10">
                            <div>
                            <h4 className="text-white font-bold group-hover:text-emerald-200 transition-colors">{pool.bill}</h4>
                        ...
                        <div className="h-full bg-emerald-500/50 shadow-[0_0_10px_rgba(16,185,129,0.4)]" style={{ width: `${(pool.cleared/pool.total)*100}%` }}></div>
                        </div>
                   </div>
               </div>
             ))}
        ...
        <section>
          ...
          <div className="space-y-3 opacity-60">
             {[
               ...
               <div key={i} className="flex justify-between items-center text-xs p-3 border-b border-white/5 bg-slate-900/20 rounded-lg hover:bg-slate-900/40 transition-colors">
