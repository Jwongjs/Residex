      ...
      <div className="w-full max-w-md bg-[#0f172a] sm:rounded-3xl rounded-t-3xl p-6 border border-white/10 shadow-2xl animate-ios-slide-up duration-500 h-[85vh] flex flex-col relative overflow-hidden">
                ...
                <div className="bg-[#0f172a] border border-white/10 p-6 rounded-2xl shadow-2xl w-full max-w-xs text-center ring-1 ring-white/20 transform animate-ios-pop-in duration-300">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button 
                            ...
                            className="flex-1 py-3.5 rounded-xl font-bold bg-gradient-to-r from-cyan-500 to-blue-600 text-white hover:shadow-lg hover:shadow-cyan-500/20 transition-all active:scale-95"
                        >
                            Create
                        </button>
                    </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar -mx-2 px-2 space-y-6">
            ...
            <div className="flex flex-col items-center gap-4">
                ...
                <input 
                    ...
                    className="w-full bg-transparent border-b border-white/20 py-2 text-center text-white font-bold text-xl focus:outline-none focus:border-cyan-500 placeholder-slate-600 transition-all"
                    autoFocus={!group}
                />
            </div>
                    ...
                    {availableFriends.map((friend, idx) => {
                         ...
                         return (
                            <button
                                ...
                                style={{ animationDelay: `${idx * 50}ms` }}
                                className={`w-full flex items-center justify-between p-3 rounded-xl transition-all duration-300 ease-ios animate-in slide-in-from-bottom-2 fade-in fill-mode-forwards active:scale-[0.98] ${isSelected ? 'bg-cyan-500/10 border border-cyan-500/30' : 'hover:bg-white/5 border border-transparent'}`}
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className={`text-sm font-medium transition-colors ${isSelected ? 'text-cyan-100' : 'text-slate-300'}`}>{friend.name}</span>
                                </div>
                                {isSelected && <Check size={16} className="text-cyan-400 animate-ios-pop-in" />}
                            </button>
                         )
                    })}
                </div>
        ...
        <div className="pt-4 mt-4 border-t border-white/10 flex gap-3">
            ...
            <button 
                ...
                disabled={!name.trim()}
                className="flex-1 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/30 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 duration-300 ease-ios"
            >
                <Save size={18} />
