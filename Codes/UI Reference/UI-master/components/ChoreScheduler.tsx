...
export const ChoreScheduler: React.FC<ChoreSchedulerProps> = ({ tenants, onBack, onExpand, isFullPage = false }) => {
  ...
  return (
    <div className={`flex flex-col animate-in fade-in duration-500 ${isFullPage ? 'bg-[#020205] h-full' : ''}`}>
      {isFullPage && (
        <div className="p-6 flex items-center justify-between border-b border-white/5 bg-black/20 backdrop-blur-md sticky top-0 z-50">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Schedule of Operations</p>
            </div>
          </div>
          <button 
            onClick={() => setShowAddModal(true)}
            className="h-10 w-10 rounded-xl flex items-center justify-center shadow-lg transition-all active:scale-95 bg-cyan-500 text-white"
          >
            <Plus size={20} />
          </button>
        </div>
      ...
      <div className={`${isFullPage ? 'flex-1 overflow-y-auto no-scrollbar p-4 sm:p-6 pb-24' : 'mb-8'}`}>
        {!isFullPage && (
          <div className="flex justify-between items-center mb-4 px-2">
            ...
            <div className="flex items-center gap-3">
                ...
                <button onClick={() => setShowAddModal(true)} className="text-[10px] font-black tracking-widest uppercase flex items-center gap-1 text-cyan-400 hover:text-cyan-300">
                    <Plus size={14} /> Add
                </button>
            </div>
          </div>
            ...
            <>
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1))} className="p-2.5 rounded-xl bg-white/5 text-slate-400 border border-white/5"><ChevronLeft size={18} /></button>
                  <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1))} className="p-2.5 rounded-xl bg-white/5 text-slate-400 border border-white/5"><ChevronRight size={18} /></button>
                </div>
              </div>
              <div className="grid grid-cols-7 mb-4 text-center text-[10px] font-black text-slate-600 uppercase">
                  ...
                  return (
                    <button
                      ...
                      className={`aspect-square rounded-2xl flex flex-col items-center justify-center relative transition-all duration-300 border
                        ${isSelected ? 'bg-cyan-500 border-cyan-400 text-white scale-110 shadow-lg' : 'bg-white/5 border-transparent text-slate-400'}`}
                    >
                      <span className="text-sm font-black">{date.getDate()}</span>
                      {hasChores && !isSelected && <div className="absolute bottom-2 w-1 h-1 rounded-full bg-cyan-400 animate-pulse" />}
                    </button>
                  );
                })}
              </div>
                ...
                return (
                  <button
                    ...
                    className={`flex-shrink-0 w-12 h-16 rounded-2xl flex flex-col items-center justify-center transition-all duration-300 border relative ${
                      isSelected ? 'bg-cyan-500 border-cyan-400 text-white scale-110 shadow-lg' : 'bg-white/5 border-transparent text-slate-500'
                    }`}
                  >
        ...
        <div className="space-y-3 min-h-[100px]">
          {filteredChores.length === 0 ? (
            ...
            filteredChores.map((chore) => {
              ...
              return (
                <div key={chore.id} className="bg-slate-800/20 backdrop-blur-md border border-white/5 rounded-[1.5rem] p-4 flex items-center justify-between group hover:border-white/20 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-[10px] font-mono text-slate-500">{chore.time}</span>
                      </div>
                    </div>
                  </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/95 backdrop-blur-md p-4 animate-in fade-in">
          <div className="w-full max-w-md bg-[#0f172a] border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 max-h-[90vh] flex flex-col">
            <div className="p-8 overflow-y-auto no-scrollbar">
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-white font-black text-2xl uppercase tracking-tighter">Schedule Task</h3>
                <button onClick={() => setShowAddModal(false)} className="h-10 w-10 bg-white/5 rounded-full flex items-center justify-center text-slate-400"><X size={20} /></button>
              </div>
              <div className="grid grid-cols-2 gap-3 mb-6">
                {CHORE_TEMPLATES.map((t) => (
                    <button key={t.name} onClick={() => setNewChore({ ...newChore, name: t.name })} className={`p-4 rounded-2xl border text-left flex items-center gap-3 transition-all ${newChore.name === t.name ? 'bg-cyan-500/20 border-cyan-500 text-white' : 'bg-white/5 border-white/5 text-slate-500'}`}>
              ...
              <div className="space-y-6">
                <input type="text" value={newChore.name} onChange={(e) => setNewChore({...newChore, name: e.target.value})} placeholder="Task Name" className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-6 text-white font-black placeholder-slate-700 focus:outline-none" />
                <div className="grid grid-cols-2 gap-4">
                  <select value={newChore.assignee} onChange={(e) => setNewChore({...newChore, assignee: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-6 text-white font-black focus:outline-none appearance-none uppercase"><option value="">Assign To</option><option value="u1">YOU</option>{tenants.map(t => (<option key={t.id} value={t.id}>{t.name.toUpperCase()}</option>))}</select>
                  <input type="time" value={newChore.time} onChange={(e) => setNewChore({...newChore, time: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-6 text-white font-black focus:outline-none" />
                </div>
                <button onClick={handleCreateChore} disabled={!newChore.name || !newChore.assignee} className="w-full py-5 rounded-[1.8rem] font-black text-xs uppercase tracking-[0.2em] shadow-2xl transition-all active:scale-95 disabled:opacity-50 bg-cyan-500 text-white">Create Task</button>
              </div>
            </div>
          </div>
        </div>
