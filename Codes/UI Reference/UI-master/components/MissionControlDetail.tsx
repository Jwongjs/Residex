...
export const MissionControlDetail: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  return (
    <div className="flex flex-col h-full bg-[#010105] animate-in slide-in-from-bottom-6 duration-500">
      <div className="p-6 flex items-center gap-4 border-b border-indigo-500/10 sticky top-0 bg-[#010105]/95 backdrop-blur-md z-10">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar pb-24">
        <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-[2.5rem] p-8 relative overflow-hidden group">
          ...
          <div className="flex flex-col gap-4 relative z-10">
            <div className="flex items-center gap-4">
                <div className="h-16 w-16 rounded-2xl bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center text-indigo-400 shadow-lg shadow-indigo-900/20">
                    <Timer size={32} />
                </div>
                <div>
                    <h2 className="text-3xl font-black text-white italic leading-none">4 Active</h2>
                    <p className="text-indigo-400 text-[10px] font-black uppercase tracking-widest mt-1">Operational Tasks</p>
                </div>
            </div>
            
            <div className="w-full bg-slate-900/50 h-1.5 rounded-full overflow-hidden mt-2">
        ...
        <section>
          ...
          <div className="bg-slate-900/60 border border-indigo-500/30 rounded-[2rem] p-6 shadow-xl relative overflow-hidden">
             <div className="absolute top-0 left-0 w-1 h-full bg-indigo-500"></div>
             <div className="flex justify-between items-center mb-4 pl-2">
                <div className="flex items-center gap-3">
                   <div className="h-10 w-10 bg-slate-800 rounded-xl flex items-center justify-center font-black text-xs text-white border border-white/5">AL</div>
                   <div>
                      <h4 className="text-white font-bold text-sm">Kitchen Sanitization</h4>
                      <p className="text-slate-500 text-[10px] uppercase font-bold">Evidence submitted 12m ago</p>
                   </div>
                </div>
                <button className="flex items-center gap-1.5 text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-500/20 hover:bg-emerald-500/20 transition-colors">
                  <Camera size={14} /> 
                  <span className="text-[9px] font-black uppercase">View</span>
                </button>
             </div>
             <div className="flex gap-3 pl-2">
                <button className="flex-1 py-3 bg-indigo-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-500/20 active:scale-95 transition-all">Verify</button>
        ...
        <section>
          ...
          <div className="space-y-4">
             {[
               ...
               <div key={i} className="flex items-center justify-between bg-slate-900/40 border border-white/5 p-4 rounded-2xl hover:bg-slate-800/40 transition-colors">
                 <div className="flex items-center gap-4">
                    <div className={`h-2 w-2 rounded-full ${op.status === 'warning' ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500'}`}></div>
                    <div>
                       <div className="text-white text-xs font-bold mb-0.5">{op.task}</div>
                       <div className="text-slate-500 text-[10px] uppercase font-bold">{op.name} is handling</div>
                    </div>
                 </div>
